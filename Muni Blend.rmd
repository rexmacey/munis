...
xlInfo<- read_xlsx(paste0(rafi.data.loc, "xlInfo.xlsx"), col_types = "text") %>% filter(version == "202307")

```

```{r initialize, include=FALSE, warning=FALSE, echo=FALSE, message=FALSE}
lookup <- read_xlsx("FundsRepresentingSegments.xlsx")
# rafi <- rafi.read.xls.v2(rafi.data.loc, xls.file)
rafi <- rafi.read.xls.v3(rafi.data.loc, xls.file, xlInfo)
segmentNames <- intersect(lookup$RAFI_Class, rafi$ret$`Asset Class`)
rafi$ret <- rafi$ret %>% filter(.$"Asset Class" %in% segmentNames)
rafi$corr <- rafi$corr %>% filter(Segment %in% segmentNames) %>% select(c("Segment", rafi$ret$`Asset Class`)) %>%
  relocate(c("Segment",rafi$ret$`Asset Class`))
rafi$cov <- rafi$cov %>% filter(Segment %in% segmentNames) %>% select(c("Segment", rafi$ret$`Asset Class`)) %>%
  relocate(c("Segment",rafi$ret$`Asset Class`))
...
# make sure corr and cov are in same order as ret
idxrow <- match(rafi$corr$Segment, rafi$ret$`Asset Class`)
rafi$corr <- rafi$corr[idxrow, ]
idxrow <- match(rafi$cov$Segment, rafi$ret$`Asset Class`)
rafi$cov <- rafi$cov[idxrow, ]


lookup[is.na(lookup$Ticker), "Ticker"] <- "NA"
for (i in 1:nrow(lookup)) {
    ticker <- lookup$Ticker[i]
    if (ticker != "NA") {
        temp <- NULL
        attempt <- 0
        while (is.null(temp) && attempt <= 5) {
            attempt <- attempt + 1
            try(
                temp <- scrapeQuoteSummary(ticker)
            )
        }
        if (is.null(temp)) {
            warning(paste("ScrapeQuoteSummary failed for",ticker))
        } else {
            lookup$Name[i] <- temp['Fund Name']    
        }
    }
}
...
tickers <- lookup$Ticker[lookup$Ticker != "NA"]
...
out <- data.frame("Asset.Class" = c(colnames(rafi$corr), "Munis")) %>% filter(Asset.Class != "Segment")
...
out.cov <- rafi$cov %>% mutate(Munis = 0) %>% add_row(Segment = "Munis")
...
fixColumnNames <- function(columnNames){
  ...
  keyWords <- c("Nominal", "Real", "Yield", "Growth")
